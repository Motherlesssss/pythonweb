...
class UserModel(db.Model):
    ...
    _password= db.Column(db.String(255),nullable=False,unique=False)
    email = db.Column(db.String(50),nullable=False,unique=True)
    avatar = db.Column(db.String(100))                                       #头像
    signature = db.Column(db.String(100))
    join_time = db.Column(db.DateTime,default=datetime.now)
    is_staff = db.Column(db.Boolean,default=False)                           #是否为员工，默认为否
    is_active = db.Column(db.Boolean,default= True)                          #是否可用，默认为是
